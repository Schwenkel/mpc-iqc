...
syn_init.sigma = ana_var.sigma;
% M1, X1, M2, X2 are only needed if M1 and M2 are free, and then we have
% only one Channel:
syn_init.X1 = (1-syn_init.sigma(1))*ana_ws.X;
syn_init.M1 = (1-syn_init.sigma(1))*ana_ws.M;
syn_init.X2 = syn_init.sigma(1)*ana_ws.X;
syn_init.M2 = syn_init.sigma(1)*ana_ws.M;
...
function [LMIs, margins, ana_var] = get_LMIs(iqc, GK, CH, rho, margin, M1M2_free)
    ...
    sigma = zeros(1,length(CH));
    for j = 1:length(CH)
        sigma(j) = CH(j).sigma;
        SDzjp_zero = min(min(S.D(z(CH(j).z),p)==0));
        if SDzjp_zero
            sigma(j) = 0;
        end
        ...
        if CH(j).gain == "e2e"
            ...
            LMI2 = pe2p_LMI2(alpha, beta, Sj, P_, X1j, X2j, M2j, gam_(j), margins(end), SDzjp_zero);
            LMIs = [LMIs, LMI1, LMI2];
        end
    end
    ...
    ana_var.gam = gam_;
    ana_var.sigma = sigma; % not really a decision variable, but might be 
                           % changed in here if SDzp==0
end
...
function LMI = pe2p_LMI2(alp, bet, S, P, X1, X2, M2, gam, margin, SDzp_zero)
    ns = length(S.OutputGroup.s);  % LMI (21) in [Schwenkel et. al., 2025]  
    nP = length(P);
    s = 1:ns;
    p = S.InputGroup.p;   np = length(p);
    z = S.OutputGroup.z;  nz = length(z);
    nw = size(S.B,2)-np;
    ...
    if SDzp_zero
        ...
        outer = [eye(nP)          zeros(nP,np+nw)
                 S.A              S.B
                 S.C(s,:)         S.D(s,:)
                 zeros(nw,nP+np)  eye(nw)];
        inner = blkdiag(X1bar-P, X2bar, M2, -alp*(gam-bet)*eye(nw));
        % Schur complement
        LM = [outer'*inner*outer    [S.C(z,:)'; S.D(z,:)'];
              S.C(z,:)  S.D(z,:)    -gam/alp*eye(nz)];
    end
