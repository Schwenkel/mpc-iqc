...
Grho.A = G.A/rho;
Grho.B = G.B/rho;
Ggen = construct_generalized_plant(Grho,syn_init.Psih);

% use correct sigma (may have been set to 0):
for j=1:length(CH)
    CH(j).sigma = syn_init.sigma(j);
end
...
if CH(1).gain == "init"  % in the initialization there is no warm start K
    ...
    syn_ws = assign_warm_start(syn_var,Ggen,syn_init);
end

[LMIs, margins] = get_LMIs(Ggen, CH, syn_var, syn_ws, eps);
...
[Krho, Ph] = obtain_controller(Ggen, syn_ws);
K = Krho;
K.A = K.A*rho;
K.B = K.B*rho;
...
function [LMIs, margins] = get_LMIs(Ggen,CH,syn_var,syn_ws, margin)
    nK = length(Ggen.A);
    CL = get_syn_CL(Ggen,syn_var);
    CL_old = get_syn_CL(Ggen,syn_ws);
...
function LMI = pe2p_LMI2(a,b,P,P11o,Xh,CL,CLo,sigma,gam,margin)
    ...
    sigma_tol=1e-5;
    if (1-sigma) < sigma_tol % i.e. sigma==1
    ...
    elseif sigma < sigma_tol % i.e. sigma==0
...
% function yes = is_SDzjp_zero(G,zj)
%     p = G.InputGroup.p;
%     y = G.OutputGroup.y;
%     yes = false;
%     if min(min(G.D(zj,p)==0))
%         if min(min(G.D(y,p)==0)) || min(min(G.D(zj,u)==0))
%             yes = true;
%         end
%     end
% end